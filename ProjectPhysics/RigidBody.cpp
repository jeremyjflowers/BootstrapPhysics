...
void RigidBody::resolveCollision(RigidBody* other)
{
	...
	//Vrel is the reverse velocity before collision
	...
	float impulse = glm::dot((-(1 + elasticity) * relativeVelocity), collisionNormal) / glm::dot(collisionNormal, (collisionNormal * (1 / massA + 1 / massB)));
